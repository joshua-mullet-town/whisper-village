...
class ParakeetTranscriptionService: TranscriptionService {
    ...
    func loadModel() async throws {
        if isModelLoaded {
            StreamingLogger.shared.log("🦜 Model already loaded, skipping")
            return
        }

        StreamingLogger.shared.log("🦜 Starting Parakeet model loading")
        ...
        do {
            ...
            if let customDirectory = customModelsDirectory {
                StreamingLogger.shared.log("🦜 Loading from custom directory: \(customDirectory.path)")
                logger.notice("🦜 Loading from custom directory: \(customDirectory.path)")
                models = try await AsrModels.downloadAndLoad(to: customDirectory)
            } else {
                StreamingLogger.shared.log("🦜 Loading from default directory")
                logger.notice("🦜 Loading from default directory")
                models = try await AsrModels.downloadAndLoad()
            }

            StreamingLogger.shared.log("🦜 Models loaded, initializing ASR manager...")
            try await asrManager?.initialize(models: models)
            isModelLoaded = true
            StreamingLogger.shared.log("🦜 Parakeet model loaded successfully")
